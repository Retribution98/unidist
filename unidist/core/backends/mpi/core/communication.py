...
class MPIState:
    ...
    def __init__(self, comm, host_comm):
        ...
        for hosted_ranks in self.topology.values():
            self.workers.extend([r for i, r in enumerate(hosted_ranks) if r != MPIRank.ROOT and i != MPIRank.MONITOR])
    ...
    def __get_topology(self):
        ...
        hosts = set([host for host, rank, host_rank in cluster_info])

        for host in hosts:
            topology[host] = []
        

        # sort by host_rank to have the
    ...
    def get_monitor_by_worker_rank(self, rank):
        for host, hosted_ranks in self.topology.items():
